...
def FHV():
    ...
    for i in ['01','02','03','04','05','06','07','08','09','10','11','12']:
        with tempfile.NamedTemporaryFile("w") as tempdir:
            ...
            # Split the dataframe into 6 dataframes
            dfs = np.array_split(df, 6)

            # Apply transformations and load each dataframe
            for i, df_half in enumerate(dfs):
                print(f"Processing dataframe {i+1}")
                
                # Data cleaning
                df_half = df_half.rename(columns={'dispatching_base_num': 'dispatching_base_id',
                ...
                df_half = df_half.drop(['special_request_flag'], axis=1)

                # change to python string for further exploration and conversion
                df_half['pickup_time'] = df_half['pickup_time'].astype('str')
                df_half['dropoff_time'] = df_half['dropoff_time'].astype('str')
                
                # Categorize trips into seasons
                conditions = [
                    (df_half['pickup_time'].between('2018-12-01 00:00:00', '2019-02-28 23:59:59')) &
                    (df_half['dropoff_time'].between('2018-12-01 00:00:00', '2019-02-28 23:59:59')),
                    (df_half['pickup_time'].between('2019-03-01 00:00:00', '2019-05-31 23:59:59')) &
                    (df_half['dropoff_time'].between('2019-03-01 00:00:00', '2019-05-31 23:59:59')),
                    (df_half['pickup_time'].between('2019-06-01 00:00:00', '2019-08-31 23:59:59')) &
                    (df_half['dropoff_time'].between('2019-06-01 00:00:00', '2019-08-31 23:59:59')),
                    (df_half['pickup_time'].between('2019-09-01 00:00:00', '2019-11-30 23:59:59')) &
                    (df_half['dropoff_time'].between('2019-09-01 00:00:00', '2019-11-30 23:59:59')),
                    (df_half['pickup_time'].between('2019-12-01 00:00:00', '2019-12-31 23:59:59')) &
                    (df_half['dropoff_time'].between('2019-12-01 00:00:00', '2019-12-31 23:59:59'))
                ]
                values = ['winter', 'spring', 'summer', 'autumn', 'winter']
                df_half['season'] = np.select(conditions, values, default='Unknown')
                
                # Load dataframe into BigQuery
                table_id = f"{project_id}.{dataset_name}.{service}_tripdata"
                job_config = bigquery.LoadJobConfig(autodetect=False)
                job = bq_client.load_table_from_dataframe(df_half, table_id, job_config=job_config)
                job.result()
